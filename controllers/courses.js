...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `User ${req.user.id} is not authorise to add a course to bootcamp ${bootcamp._id} `,
        401
      )
    );
  }
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `User ${req.user.id} is not authorise to update a course ${course._id} `,
        401
      )
    );
  }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(
      new ErrorResponse(
        `User ${req.user.id} is not authorise to delete a course ${course._id} `,
        401
      )
    );
  }
